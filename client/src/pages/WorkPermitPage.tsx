...
import { workPermitPages } from "@shared/workPermits";
import SEOHead from "@/components/SEOHead";
import { Button } from "@/components/ui/button";
import { Link } from "wouter";
import { Clock, DollarSign, CheckCircle, HelpCircle, ArrowRight, Phone, Mail } from "lucide-react";
...
export default function WorkPermitPage() {
  ...
  const pageData = workPermitPages.find(page => page.slug === slug);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-6">The work permit page you're looking for doesn't exist.</p>
          <Button asChild>
  ...
  return (
    <>
      <SEOHead
        ...
        canonicalUrl={`${window.location.origin}/work-permits/${pageData.slug}`}
      />
      ...
      <section className="py-20 bg-secondary/30 dark:bg-secondary/30" data-testid="eligibility-process-section">
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
              ...
              <ul className="space-y-4">
                {pageData.eligibility.requirements.map((requirement, index) => (
                  <li key={index} className="flex items-start gap-3" data-testid={`requirement-${index}`}>
                    <div className="w-2 h-2 bg-accent dark:bg-accent rounded-full mt-3 flex-shrink-0"></div>
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                <ArrowRight className="w-6 h-6 text-accent dark:text-accent" />
              ...
              <div className="space-y-6">
                {pageData.process.steps.map((step, index) => (
                  <div key={index} className="flex gap-4" data-testid={`process-step-${index}`}>
                    <div className="w-8 h-8 bg-accent dark:bg-accent text-white dark:text-white rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">
                      {step.step}
                    </div>
                    <div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            >
              <div className="flex items-center justify-center gap-3 mb-4">
                <HelpCircle className="w-6 h-6 text-accent dark:text-accent" />
                <h2 className="text-3xl font-serif font-bold text-primary dark:text-primary" data-testid="faq-title">
                  Frequently Asked Questions
                </h2>
              </div>
              <p className="text-lg text-muted-foreground dark:text-muted-foreground">
                Get answers to common questions about {pageData.title.toLowerCase()}
              </p>
            </motion.div>
      ...
      {/* CTA Section */}
      <section className="py-20 bg-primary dark:bg-primary" data-testid="cta-section">
        <div className="container mx-auto px-6 text-center">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-white dark:text-white mb-6" data-testid="cta-title">
              Ready to Apply for Your {pageData.title}?
            </h2>
            <p className="text-xl text-white/90 dark:text-white/90 mb-8" data-testid="cta-description">
              Don't navigate the complex immigration process alone. Our experienced lawyers will guide you through every step to maximize your chances of success.
            </p>
            ...
            <div className="flex items-center justify-center gap-6 text-white/80 dark:text-white/80">
              <div className="flex items-center gap-2">
                <Mail className="w-4 h-4" />
                <span className="text-sm">info@ourlawservices.com</span>
              </div>
